...
A [Claude Code](https://docs.anthropic.com/en/docs/claude-code) skill for multi-source technical research. Dispatches parallel subagents to gather intelligence from three sources and synthesizes findings into a unified report.
...
- **Claude Code** with access to the Task tool (for dispatching subagents)
...
├── references/
│   ├── subagent_templates.md         # Complete prompt templates for each subagent
│   └── query_strategies.md           # Grok query crafting strategies (5 patterns)
└── scripts/
    ├── grok_preflight.sh             # Pre-flight check for Grok availability
    └── grok_update_status.sh         # Track Grok login state
```

## Optional: ask-grok Skill

This skill optionally delegates to the [ask-grok](https://github.com/nicobailon/ask-grok-claude-code) skill for Grok login state tracking. If `ask-grok` is not installed, the skill falls back to checking login via Playwright snapshots each time.
